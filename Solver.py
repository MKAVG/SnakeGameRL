...
class Solver:
    def __init__(self, game = None):
        ...
        self.moves =[]
        self.directionArray = [self.directions.UP,self.directions.DOWN,self.directions.LEFT,self.directions.RIGHT]
    ...
    def setCurrentMoves(self,moves):
        self.moves = moves
    ...
    def drawTempState(self):
        ...
        for component in self.tempBodyLocations:
            ...
            pygame.draw.rect(self.myDisplay, B, (   #G
            component[0] * self.game.displaySize, component[1] * self.game.displaySize, self.game.displaySize,
            self.game.displaySize))

        for i in range(self.game.map.cols):
            pygame.draw.rect(self.myDisplay, R,
                             (i * self.game.displaySize, 0, self.game.displaySize, self.game.displaySize))
        for i in range(self.game.map.rows):
            pygame.draw.rect(self.myDisplay, R,
                             (0, i * self.game.displaySize, self.game.displaySize, self.game.displaySize))
        for i in range(self.game.map.cols):
            pygame.draw.rect(self.myDisplay, R, (
            i * self.game.displaySize, self.game.displaySize * self.game.map.rows - self.game.displaySize,
            self.game.displaySize, self.game.displaySize))
        for i in range(self.game.map.rows):
            pygame.draw.rect(self.myDisplay, R, (
            self.game.displaySize * self.game.map.cols - self.game.displaySize, i * self.game.displaySize,
            self.game.displaySize, self.game.displaySize))

        xFood, yFood = self.game.map.food
        pygame.draw.rect(self.myDisplay, R, (
        xFood * self.game.displaySize, yFood * self.game.displaySize, self.game.displaySize, self.game.displaySize))
    ...
    def drawState(self):
        ...
        while time.time() < startTime+self.game.waitTime:
            ...
            for component in self.game.snake.bodyLocations:
                # print(component)
            # component = self.game.snake.bodyLocations[0]
                pygame.draw.rect(self.myDisplay,B,(component[0]*self.game.displaySize,component[1]*self.game.displaySize,self.game.displaySize,self.game.displaySize))
            ...
            xFood,yFood = self.game.map.food
            pygame.draw.rect(self.myDisplay,R,(xFood*self.game.displaySize,yFood*self.game.displaySize,self.game.displaySize,self.game.displaySize))
